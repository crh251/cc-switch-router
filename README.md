...
7. `/model` 实时显示映射：ccs 通过 `CLAUDE_CONFIG_DIR` 把终端 claude 的配置目录重定位到 `~/.cc-switch/claude-home/<供应商>/`（共享子项软链回 `~/.claude`），其 `settings.json` 就是「用户级」设置、claude 热加载。`ccs_router.rs` 新增 `regenerate_claude_settings_files()`，在供应商增删改及启动时自动重生成该文件（内容 = 用户真实 settings.json + env 接管覆盖：`_MODEL`=官方档位别名、`_MODEL_NAME`=映射名）——ccs 终端里改映射后 `/model` 实时刷新，选中即发送官方别名、由代理按关键词实时路由，全程无需重启
...
构建时会自动把 commit id 盖进版本：app 内「关于」标签页显示 `3.20.1+<短commit>`，关于窗口括号内显示 `<短commit>, ccs <版本>`。ccs 版本**写死在脚本源码里**（最后一次逻辑修改的时间戳，逻辑更新时手动同步），CI 只读取它盖进关于窗口，并校验补丁内嵌脚本与仓库顶层脚本逐字节一致（不一致构建直接失败）。
...
```bash
ccs                    # 官方直连（api.anthropic.com，不经本地路由；插件全加载）
ccs claude             # 同上（别名 official / anthropic）
